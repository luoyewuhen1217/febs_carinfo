var validator;
var $carInfoAddForm = $("#carInfo-add-form");
// var $rolesSelect = $carInfoAddForm.find("select[name='rolesSelect']");
// var $roles = $carInfoAddForm.find("input[name='roles']");
...
$(function () {
    ...
    $("#carInfo-add .btn-save").click(function () {
        var name = $(this).attr("name");
        getDept();
        var validator = $carInfoAddForm.validate();
        var flag = validator.form();
        if (flag) {
            if (name === "save") {
                $.post(ctx + "user/add", $carInfoAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
            ...
            if (name === "update") {
                $.post(ctx + "user/update", $carInfoAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
                        $MB.refreshTable("userTable");
...
function closeModal() {
    ...
    $carInfoAddForm.find("input[name='username']").removeAttr("readonly");
    $carInfoAddForm.find(".user_password").show();
    $carInfoAddForm.find("input[name='status']").prop("checked", true);
...
//     validator = $carInfoAddForm.validate({
//         rules: {
//             username: {
//                 required: true,
//                 minlength: 3,
